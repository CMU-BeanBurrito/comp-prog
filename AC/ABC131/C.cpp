...
// PUT GLOBALS HERE
ll num_mult(ll l, ll r, ll x) // how many multiples of x in [l, r]
{
    ll first = l%x == 0 ? l : l+x-(l%x);
    ll last = r%x == 0 ? r : r - (r%x);

    if (last < first) return 0;
    return 1 + ((last-first)/x);
}

void solve() {
    ll a, b, c, d; cin >> a >> b >> c >> d;

    printf("%lld\n", (b-a+1) - (num_mult(a,b,c)+num_mult(a,b,d) - num_mult(a,b,lcm(c, d))));
}
