...
#define MAXH 1000
#define MAXW 1000
vector<vector<int>> g (MAXH*MAXW);
vector<bool> vis (MAXH*MAXW, false);
vector<int> dist (MAXH*MAXW, INT_MAX);
queue<int> q;
...
void solve() {
    ...
    for (int i = 0; i < h; i++)
    {
        for (int j = 0; j < w; j++)
        {
            // walls should not neighbor any cells
            if (s[i][j] == '#') continue;

            // current cell's location in the grid
            int u = i*w + j;

            // humidifiers are sources for BFS
            if (s[i][j] == 'H')
            {
                dist[u] = 0;
                q.push(u);
            ...
            // check adjacent cells to see if they should be neighbors
            // only need to create one-way edges, the other direction will also be created
            if (i > 0 && s[i-1][j] != '#')
            {
                int v = (i-1)*w + j;
                g[u].push_back(v);
