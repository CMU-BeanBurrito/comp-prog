...
void solve() {
    ...
    while(q--)
    {
        ...
        int hand = (h == 'L' ? left : right);
        int other = (h == 'L' ? right : left);

        if (dist(hand, t, n) < dist(hand, other, n)) // dest is closer than other hand, go directly
        {
            ans += dist(hand, t, n);
        }
        else if (dist(hand, other, n) + dist(other, t, n) == dist(hand, t, n)) // other hand is closer than dest, 
        {
            ans += n - dist(hand, t, n);
        }
        else
