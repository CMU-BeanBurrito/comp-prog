...
void solve() {
    ...
    vector<int> b (n+1, -1); // b[i] = index in a where i has its first occurrence
    vector<int> c (n+1, -1); // c[i] = index in a where i has its second occurrence
    ...
    for (int i = 0; i < 2*n-1; i++)
    {
        ...
        pos.push_back(b[a[i]]);
        pos.push_back(c[a[i]]);
        pos.push_back(b[a[i+1]]);
        pos.push_back(c[a[i+1]]);

        if (b[a[i]] + 1 == c[a[i]]) continue;
        if (b[a[i+1]] + 1 == c[a[i+1]]) continue;

        sort(pos.begin(), pos.end());

        if (pos[0] + 1 == pos[1] && pos[2] + 1 == pos[3]) ans.insert(minmax(a[i], a[i+1]));
    }
