...
void solve() {
    ...
    for (int i = 0; i < n; i++)
    {
        if (s[i] != '?') // if s[i] is 'o' or '.', t[i] = s[i]
        {
            t[i] = s[i];
        }
        else
        {
            if (i > 0 && s[i-1] == 'o') // if adjacent to an 'o', have to be a '.'
            {
                t[i] = '.';
            }
            else if (i < n-1 && s[i+1] == 'o') // if adjacent to an 'o', have to be a '.'
            {
                t[i] = '.';
            }
            else
    ...
    else
    {
        ...
        vector<pair<int, int>> blocks; // start, length of ? blocks
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            {
                blocks.push_back(make_pair(i-blk, blk));
                blk = 0;
            }
        }

        if (blk > 0) blocks.push_back(make_pair(n-blk, blk));

        int mx = 0;

        for (auto p : blocks) mx += (p.second+1)/2;
        ...
        if (mx == need)
        {
            for (auto p : blocks)
            {
                if (p.second % 2 == 1)
                {
                    for (int i = p.first; i <= p.first + p.second; i += 2) t[i] = 'o';
                    for (int i = p.first+1; i <= p.first + p.second; i += 2) t[i] = '.';
                }
            }
        }
