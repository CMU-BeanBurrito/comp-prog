...
#define MAXN 100005
#define MAXW 10
vector<vector<int>> g (MAXN * MAXW, vector<int>());
vector<bool> vis (MAXN * MAXW, false);
vector<int> indeg (MAXN * MAXW, 0);
vector<string> hol (MAXN);
int n, m, w;
queue<int> q;
vector<int> path;
...
void solve() {
    ...
    cin >> w;
    for (int i = 0; i < n; i++) cin >> hol[i];

    // add possible moves for staying in the same city
    for (int i = 0; i < n; i++)
    {
        for (int d = 0; d < w; d++)
        {
    ...
    // add possible moves for changing cities
    for (int i = 0; i < m; i++)
    {
        for (int d = 0; d < w; d++)
        {
            ...
            if (hol[s][d] == 'o' && hol[t][(d+1) % w] == 'o')
            {
                g[s*w + d].push_back(t*w + (d+1) % w);
            }

            if (hol[t][d] == 'o' && hol[s][(d+1) % w] == 'o')
            {
                g[t*w + d].push_back(s*w + (d+1) % w);
            }
        }
    }
