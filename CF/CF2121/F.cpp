...
void solve() 
{
    ...
    for (auto v : vecs)
    {
        ...
        for (int i = 1; i < m; i++) p[i] = p[i-1] + v[i];

        map<ll, vector<int>> mp;

        for (int i = 0; i < m; i++) mp[p[i]].push_back(i);

        for (int i = 0; i < m; i++)
        {
            if (nxt[i] == -1) break;
            ll key = 0LL + s + p[i] - v[i];
            ans += (mp[key].end() - lower_bound(mp[key].begin(), mp[key].end(), nxt[i]));
        }
    }
