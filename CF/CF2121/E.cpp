...
void solve() 
{
    ...
    // leading 0s
    while(lv[idx] == 0 && rv[idx] == 0) idx++;
    
    // digit we choose will equal both numbers
    while(idx < 9 && lv[idx] == rv[idx])
    {
        idx++;
        ans += 2;
    }

    // digit we choose will equal one number
    if (idx < 9 && (lv[idx] == rv[idx]-1))
    {
        idx++;
        ans++;
    }

    while(idx < 9 && (lv[idx] == 9 && rv[idx] == 0))
    {
        idx++;
        ans++;
    }
