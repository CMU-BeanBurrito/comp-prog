...
void solve() {
    ...
    for (int i = 0; i < 2*n; i++)
    {
        if (a[i][1] == 'C') cards[0].push_back(a[i]);
        if (a[i][1] == 'D') cards[1].push_back(a[i]);
        if (a[i][1] == 'H') cards[2].push_back(a[i]);
        if (a[i][1] == 'S') cards[3].push_back(a[i]);
    }

    for (int i = 0; i < 4; i++) sort(cards[i].begin(), cards[i].end());

    int num_trump = cards[trump].size();
    int unpaired = 0;

    for (int i = 0; i < 4; i++)
    {
        if (i == trump) continue;
        unpaired += cards[i].size() % 2;
    }

    if (unpaired > num_trump)
    {
        printf("IMPOSSIBLE\n");
        return;
    }
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        for (int j = 0; j < (int)cards[i].size()/2; j++)
        {
            cout << cards[i][2*j] << " " << cards[i][2*j+1] << endl;
        }

        if (cards[i].size() % 2 == 1)
        {
            cout << cards[i][cards[i].size()-1] << " " << cards[trump][trump_idx] << endl;
            trump_idx++;
        }
    }

    for (int i = trump_idx; i < (int)cards[trump].size(); i += 2)
    {
        cout << cards[trump][i] << " " << cards[trump][i+1] << endl;
    }
}
