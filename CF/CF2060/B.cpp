...
void solve() {
    ...
    for (int i = 1; i <= n; i++) sort(cards[i].begin(), cards[i].end());

    vector<pair<int, int>> order;

    for (int i = 1; i <= n; i++) order.push_back(make_pair(cards[i][0], i));
