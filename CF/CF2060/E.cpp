...
// PUT GLOBALS HERE
int maxn = 200'001;
vector<vector<int>> f (maxn, vector<int> ());
vector<vector<int>> g (maxn, vector<int> ());
vector<vector<int>> f2 (maxn, vector<int> ());
vector<int> f2mp (maxn, -1);
vector<bool> f2vis (maxn+1, false);
vector<int> gmp (maxn, -1);
vector<bool> gvis (maxn+1, false);

void gdfs(int u, int ccid)
{
    gvis[u] = true;
    for (auto v : g[u])
    {
