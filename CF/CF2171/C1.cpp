 ...
 * Strategy:
 * 
 * We can see that the highest score either player can get is 1.
 * 
 * First, determine whether or not the game is a tie.
 * - If the total number of 1s in the game is even, the game is a tie no matter what moves are made.
 * - Both players will have 1 or both players will have 0 at the end of the game.
 * 
 * Otherwise, the sum of 1s in the game is odd and there will be one winner.
 * 
 * Track how many 1s each player possesses at all times (starting with the initial amount).
 * 
 * Traverse the arrays - a player will swap if they currently have an even number of 1s.
 * - Obviously, if a[i] == b[i], they cannot swap
 * 
 * The winner is the player who has an odd number of 1s at the end.
 */
...
void solve() {
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            if (a1 % 2 == 0 && a[i] != b[i]) // Ajisai has an even number but can swap for an odd (winning move)
            {
                a1++;
                b1--;
            }
        ...
        {
            if (b1 % 2 == 0 && b[i] != a[i]) // Mai has an even number but can swap for an odd (winning move)
            {
                a1--;
                b1++;
            }
