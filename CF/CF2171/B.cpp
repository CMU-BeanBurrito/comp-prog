 ...
 * b[n-1] = b[n-1]-b[n-2]
 * 
 * Then, b[0] + b[1] ... + b[n-1] = a[n-1] - a[0].
 * 
 * So, we are trying to minimize |a[n-1] - a[0]|, this will be the minimized answer.
 * 
 * Any -1 that is not on index 0 or n-1 does not matter.
 * So set them to 0 to make the lexicographically minimum answer.
 * 
 * Case 1a: first and last elements are equal, but not -1.
 * - can't fill first/last, so answer is abs(last-first)
 * 
 * Case 1b: first and last elements are equal, and both are -1.
 * - Answer is 0, fill first and last as 0.
 * 
 * Case 2a: First is -1, last is not -1.
 * - Answer is 0, set first = last
 * 
 * Case 2b: First is not -1, last is -1.
 * - Answer is 0, set last = first
 */
...
void solve() {
    ...
    {
        printf("0\n");
        for (int i = 0; i < n; i++) printf("%d ", (a[i] == -1) ? 0 : a[i]);
        printf("\n");
    }
    else if (a[n-1] == -1)
    {
        printf("0\n");
        for (int i = 0; i < n-1; i++) printf("%d ", (a[i] == -1) ? 0 : a[i]);
        printf("%d\n", a[0]);
    }
    else if (a[0] == -1)
    {
        printf("0\n");
        printf("%d ", a[n-1]);
        for (int i = 1; i < n; i++) printf("%d ", (a[i] == -1) ? 0 : a[i]);
        printf("\n");
    }
    else
    {
