 ...
 * Strategy:
 * 
 * Be sure to understand the solution for the easy version.
 * 
 * *bit k = the kth most significant bit. e.g. bit 3 is in the "8s" place
 * 
 * Note that the "max bit" is 20, as the max value.
 * 
 * We start at bit 20 and work our way down, playing a new instance of the game at each bit.
 * Each of these instances are independent. If a game is tied, we move to the next bit.
 * 
 * As soon as a player wins an instance of the game, they win, as any bit trumps all the other bits below it.
 * 
 */
...
void solve() {
    ...
    vector<int> a1 (MAXB, 0);
    vector<int> b1 (MAXB, 0);
    ...
    for (int bit = 0; bit < MAXB; bit++)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            if ((a[i] & mask) != 0) a1[bit]++;
            if ((b[i] & mask) != 0) b1[bit]++;
        }
    }

    for (int bit = MAXB-1; bit >= 0; bit--)
    {
        // This bit is a tie. Move to the next most significant bit.
        if ((a1[bit] + b1[bit]) % 2 == 0) continue;
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            {
                if (a1[bit] % 2 == 0 && ((a[i] & mask) != (b[i] & mask)))
                {
                    a1[bit]++;
                    b1[bit]--;
                }
            ...
            {
                if (b1[bit] % 2 == 0 && ((b[i] & mask) != (a[i] & mask)))
                {
                    a1[bit]--;
                    b1[bit]++;
                }
    ...
    // All bits were tied, game is tied
    printf("Tie\n");
}
