...
void solve() 
{
    ...
    map<char, char> mpf; // mpf[x] = y, y is child of x
    map<char, char> mpb; // mpb[x] = y, y is parent of x
    ...
    {
        char c = o[i];
        if (mpb[c] == '0') // c has no parent yet
        {
            /**
             * Rules for choosing c2 as parent of c:
             * 
             * c and c2 must be different (or we would create a cycle of size 1)
             * c2 cannot be a parent already (or c2 would have 2 children)
             * we cannot create a cycle of any size except 26.
             */
            for (char c2 = 'a'; c2 <= 'z'; c2++)
            {
                if (c2 == c) continue;
                if (mpf[c2] != '0') continue;
                ...
                if (!good) // cycle smaller than 26 created, undo and try next char
                {
                    mpf[c2] = '0';
                    mpb[c] = '0';
                    continue;
