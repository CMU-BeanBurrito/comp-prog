...
void solve() 
{
    ...
    int ptr = 0; // next char to be colored
    ...
    while(ptr != t.length())
    {
        ...
        for (int i = 0; i < n; i++) // try s[i]
        {
            //printf("TRYING STRING %d\n", i);
            for (int idx = max(0, (int)(ptr-s[i].length()+1)); idx <= ptr; idx++)
            {
                //printf("TRYING START %d\n", idx);
                if (t.substr(idx, s[i].length()) == s[i]) // works
                {
                    //printf("WORKED\n");
                    if ((int)(idx + s[i].length()) > bestp)
                    {
                        //printf("BEST\n");
                        best = i;
                        bestp = idx + s[i].length();
                    }
                }
            }
        }
        ...
        ans.push_back(make_pair(best+1, bestp-s[best].length()+1));
