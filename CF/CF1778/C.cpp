...
void solve() {
    ...
    for (int i = 0; i < n; i++) acset.insert(a[i]);

    for (char c : acset) ac.push_back(c);

    map<char, int> mp;
    for (int i = 0; i < ac.size(); i++) mp[ac[i]] = i;

    vector<int> mask (acset.size(), 0);

    for (int i = acset.size()-1; i >= 0 && k > 0; i--, k--) mask[i] = 1;
    ...
    do
    {
        ...
        for (int i = 0; i < n; i++)
        {
            if (a[i] == b[i] || mask[mp[a[i]]] == 1)
            {
                run++;
            }
            else
            {
                score += 1LL * run * (run+1) / 2;
                run = 0;
            }
        }
