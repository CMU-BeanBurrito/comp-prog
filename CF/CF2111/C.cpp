...
void solve() 
{
    ...
    // we will do at most one leftwards operation
    // we will do at most one rightwards operation
    // cost for using an element is element times (n - longest streak)
    ...
    for (int i = 1; i < n; i++)
    {
        ...
        {
            run++;
            if (i == n-1) mp[a[i]] = max(mp[a[i]], run);
        }
        else
        {
            mp[a[i-1]] = max(mp[a[i-1]], run);
            run = 1;
            if (i == n-1) mp[a[i]] = max(mp[a[i]], run);
        }
    }
    ...
    for (auto p : mp)
    {
        ans = min(ans, 1LL * p.first * (n-p.second));
    }
