...
void solve() 
{
    ...
    // of the lowest 2^(ones) teams, only the greatest can win
    // of the highest 2^(zeros) teams, only the least can win
    // everyone else can win
    for (int i = 1 << ones; i <= (1 << n) - (1 << zeros) + 1; i++) printf("%d ", i);
    printf("\n");
}
