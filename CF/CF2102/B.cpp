...
void solve() {
    ...
    /**
     * Suppose we are making a[0] positive in our final array, a[0] = abs(a[0])
     * 
     * The numbers with lesser magnitude will all be less than a[0], no matter their sign.
     * So, if there are more than (n-1)/2 of them, abs(a[0]) will not be the median.
     * If the number of elements with magnitude < abs(a[0]) is less than (n-1)/2?
     * We can take elements that are greater than abs(a[0]) and make them negative to "push up" a[0] to be the median.
     * 
     * Suppose we are making a[0] negative in our final array, a[0] = -abs(a[0])
     * 
     * The numbers with lesser magnitude will all be greater than a[0], no matter their sign.
     * So, if there are more than (n/2) of them, abs(a[0]) will not be the median.
     * If the number of elements with magnitude < abs(a[0]) is less than (n/2)?
     * We can take elements with greater magnitude than abs(a[0]) and make them positive to "push down" a[0] to be the median.
     */
