...
void solve() {
    ...
    for (int i = 0; i < n; i += 2) ans[i] = even[i/2];
    for (int i = 1; i < n; i += 2) ans[i] = odd[i/2];

    vector<int> tmp (n);

    for (int i = 0; i < n; i++) tmp[i] = ans[i];

    ll ipar = mergesort(a, 0, n-1);
    ll fpar = mergesort(tmp, 0, n-1);

    if (fpar % 2 != ipar % 2) swap(ans[n-1], ans[n-3]);
