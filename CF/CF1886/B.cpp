...
void solve() {
    ...
    // if light circles must overlap, the radius is at most half the distance between the lamps.

    double r = sqrt((ay-by)*(ay-by) + (ax-bx)*(ax-bx))/2;

    // start and end must each be within one of the circles. each point is assigned to whichever circle it's closer to.

    double ahome = sqrt((py-ay)*(py-ay) + (px-ax)*(px-ax));
    double bhome = sqrt((py-by)*(py-by) + (px-bx)*(px-bx));
    double awork = sqrt((ax*ax) + (ay*ay));
    double bwork = sqrt((bx*bx) + (by*by));

    double ans;

    if (ahome < bhome && awork < bwork)
    {
        ans = max(ahome, awork);
    }
    else if (bhome < ahome && bwork < awork)
    {
        ans = max(bhome, bwork);
    }
    else
    {
        ans = max(min(ahome, bhome), min(awork, bwork));
        ans = max(ans, r);
    }
