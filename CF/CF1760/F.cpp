...
// PUT GLOBALS HERE
vector<int> a (200000);
vector<ll> pref (200001);
ll n, c, d;

bool check(ll k) {
    ll rounds = d/(k+1); // how many times can we do each quest
    
    ll coinspr = pref[min(n, k+1)]; // coins per round
    
    ll lo = d % (k+1); // leftover days after last complete cycle
    lo = min(lo, n); // can do this many quests after the last cycle
    
    if (pref[lo] + coinspr*rounds < c) {
        return false;
    } else {
        return true;
    }
...
void solve() {
    ...
    int cycle = min(n, d); // do as many quests as possible, but only once each
    
    if (pref[cycle] >= c) { // doing each quest once is enough, can win with any cooldown
        printf("Infinity\n");
    } else if (a[0] * d < c) { // doing the best quest everyday not enough, cannot win with any cooldown
        printf("Impossible\n");
    } else {
        int l = 0;
        int r = d; // minimum k that fails. we know d fails, otherwise we would've answered "Infinity".
