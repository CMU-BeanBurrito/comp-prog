...
void solve() {
    ...
    for (int i = 1; i <= n; i++)
    {
        ...
        if (g[i].size() > 1) i--; // need to remove more edges from this node
    }
    ...
    {
        if (i == base) continue;
        if (g[base].find(i) != g[base].end()) continue;
        
        int base2 = *g[base].begin();

        if (g[i].size() == 0)
        {
            ops.push_back({base, base2, i});
            g[base].erase(base2);
            g[base].insert(i);
            g[base2].erase(base);
            g[base2].insert(i);
