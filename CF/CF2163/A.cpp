...
/**
 * Strategy: Souvlaki, Kalamaki
 * 
 * Souvlaki needs to ensure that on Kalamaki's turn, the two numbers to be swapped or equal.
 * That way, no matter what Kalamaki does, those 2 numbers will be sorted.
 * 
 * Note that problem uses 1-indexing, this implementation uses 0-indexing.
 * So with 0-indexing, we need a[1] == a[2], a[3] == a[4], etc.
 * After re-arranging, the array will have the form A B B C C D D E E....
 * 
 * To check, sort the array and check the indices used on Kalamaki's turns.
 */
...
void solve() {
    ...
    for (int i = 1; i < n-1; i += 2) // check [1, 2], [3, 4], etc.
    {
        /**
         * a[i] < a[i+1] (since we sorted), Kalamaki can switch and win.
         * 
         * if a[i] > a[i+1] (which can't happen with the sort we used), then Kalamaki could skip his turn and win.
         * 
         * therefore, Souvlaki needs them to be equal
         */
        if (a[i] != a[i+1])
        {
            printf("NO\n");
