...
void solve() 
{
    ...
    for (int i = 0; i <= n; i++)
    {
        // p[i] should be at most floor i/2
        // p[n] - p[i] should be at least ceil(n-i)/2
        if (p[i] > i/2) continue;
        if (p[n] - p[i] < divceil(n-i, 2)) continue;

        int dist = abs(n - 2*i); // actually double the distance to handle rounding issues
