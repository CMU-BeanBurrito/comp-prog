 ...
 * Strategy:
 * 
 * We are trying to get every element to be a multiple of q.
 * An element can become a multiple of q if:
 * - it already is one
 * - It is at least 4q. Split element as x = q + q+r + 2q.
 * 
 * Otherwise, this element cannot be made into multiplies of q under the given rules.
 * We can tolerate up to k of these elements (we will erase them)
 * 
 * We cannot binary search on q, as all valid q are not less than all invalid q.
 * So, we need to check every possible q.
 * 
 * Since all elements are <= n, we can make a full frequency "map".
 * Then, make a prefix sum of this map.
 * 
 * Now, to check a value of q, we use the prefix sum to quickly count all elements >= 4q good.
 * Then, of the elements < 4q, only q, 2q, and 3q will be good. Use the frequency map to count these.
 * If we have at least n-k good elements, then this q is valid.
 * 
 * So, for each value of q in [1, n], we query the prefix sum once and the frequency map 3 times.
 * Each of these queries is O(1) due to the vector construction. So this is O(n).
 */
...
void solve() {
    ...
    {
        int good = 0;
        if (n >= 4*i) good = n-p[4*i-1];

        for (int j = i; j <= n && j < 4*i; j += i) good += f[j];
