...
void solve() 
{
    ...
    {
        int x = a[i];
        int yidx = lower_bound(a.begin()+i+1, a.end(), n-a[i]) - a.begin();
        if (yidx == m) continue;

        ll pairs = 0LL + m-yidx;
        ans += p[m-1]-p[yidx-1] + pairs*(x+1-n);
    }
    ...
    /**
     * two paints A, B
     * if cnt(A) + cnt(B) < n, 0 ways for this pair
     * 
     * if we start with paint A
     * - X planks painted A, X is max(1, N-B) to min(n-1, A)
     * 
     * if we start with paint B
     * - Y planks painted B, Y is max(1, N-A) to min(n-1, B)
     * 
     * - sort, binary search + prefix sum to do all pairs for a single color
     */
}
