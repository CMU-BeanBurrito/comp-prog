...
/**
 * Strategy: Each connected component must be complete.
 * 
 * For each vertex, find the size of its connected component.
 * If the size of connected component is k, the degree of all its vertices should be k-1.
 */
...
void solve() {
    ...
    for (int i = 1; i <= n; i++)
    {
        if (!vis[i]) // new connected component
        {
            cr[i] = i; // this vertex will be the canonical representative
            dfs(i);
        }
    }
    ...
    for (int i = 1; i <= n; i++) mp[cr[i]]++;

    // check that each vertex's degree is appropriate as described above
    for (int i = 1; i <= n; i++)
    {
        if ((int)g[i].size() != mp[cr[i]]-1)
        {
            printf("NO\n");
            return;
        }
