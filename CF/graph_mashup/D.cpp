...
/**
 * Attempt to bipartition a connected component.
 * 
 * return true if successful
 * return false if fail
 * 
 * bipartition is recorded in c; c[i] = {0, 1}
 */
bool bfs()
{
    while(!q.empty())
...
void solve() {
    ...
    // run BFS to bipartition each connected component
    for (int i = 1; i <= n; i++)
    {
        if (!vis[i]) // if visited, this connected component is already processed
        {
            // kick off BFS
            q.push(i);
            vis[i] = true;
            ...
            // if bipartition fails, there is no solution
            if (!bfs())
            {
                printf("-1\n");
                return;
    ...
    // construct the vertex covers using c
    for (int i = 1; i <= n; i++)
    {
        if (c[i] == 0) a.push_back(i);
        if (c[i] == 1) b.push_back(i);
    }
