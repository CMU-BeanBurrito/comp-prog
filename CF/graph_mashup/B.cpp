...
/**
 * We need to bribe one person/node from each connected component (CC).
 * 
 * Obviously, we should choose this to be the cheapest person in the CC.
 */
...
// returns the lowest cost person in the CC.
int dfs(int u)
{
    int mn = c[u];
    vis[u] = true;
...
void solve() {
    ...
    for (int i = 1; i <= n; i++)
    {
        if (vis[i]) continue; // if a node is visited, its entire CC is visted already
