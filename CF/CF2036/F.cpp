...
// PUT GLOBALS HERE
ll xorcont(ll l, ll r) // xor of a continuous segment, inclusive
{
    ll x = 0;
    if ((l-1) % 4 == 0) x = l-1;
    if ((l-1) % 4 == 1) x = 1;
...
void solve() {
    ...
    ll li = ((l % mod <= k) ? l/mod : l/mod + 1); // first interesting number right shifted by i bits
    ll ri = ((r % mod >= k) ? r/mod : r/mod - 1); // last interesting number right shifted by i bits

    if (li <= ri) // at least 1 interesting num
    {
        ans ^= (xorcont(li, ri) * mod);

        // even num of interesting num
        if ((ri-li+1) % 2 == 1) ans ^= k;
    }
