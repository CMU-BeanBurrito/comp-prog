...
int GEQbinsearch(int l, int r, ll key) // first index >= key
{
...
int LEQbinsearch(int l, int r, ll key) // last index <= key
{
...
void solve() {
    ...
    // pair should be in the range [sum-y, sum-x]
    ...
    {
        ll key1 = sum-y-a[i];
        int idx1 = GEQbinsearch(i+1, n-1, key1);
        
        ll key2 = sum-x-a[i];
        int idx2 = LEQbinsearch(i+1, n-1, key2);

        if (idx1 == n || idx2 == i) continue; // can't be paired with anything
