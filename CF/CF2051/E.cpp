...
int LTbinsearch(int l, int r, int key, vector<int>& v) // last index < key
{
    if (v[l] >= key) return -1;
...
void solve() {
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        int first = LTbinsearch(0, n-1, test, a);
        int second = LTbinsearch(0, n-1, test, b);

        int positive = n-first-1;
        int neutral = second+1;
        int negative = n-neutral-positive;

        if (negative > k) continue;

        best = max(best, 1LL*test*(positive+negative));
    }
