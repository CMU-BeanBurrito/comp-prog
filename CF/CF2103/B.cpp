...
void solve() {
    ...
    // if only did at most 1 flip, cannot reduce (string is either all 0, all 1, or some 0s followed by some 1s)
    // else if there is any pattern 1....0....1, we can reduce by 2
    // else we can reduce by 1 (some 0s followed by some 1s followed by some 0s)

    if (flip0 == 0)
    {
        printf("%d\n", ops);
    }
    else if (flip0 >= 1 && flip1 >= 2)
    {
        printf("%d\n", ops-2);
    }
    else // flip0 = 1, flip1 = 1
    {
        printf("%d\n", ops-1);
    }
}
