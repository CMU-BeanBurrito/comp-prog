...
void solve() 
{
    ...
    vector<vector<pair<int, int>>> color (MAXN, vector<pair<int, int>> ());
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            int x; cin >> x; x--;
            color[x].push_back(make_pair(i, j));
        }
    }
    ...
    for (int i = 0; i < MAXN; i++)
    {
        ...
        sort(color[i].begin(), color[i].end()); // sort by row index
        sum = 1LL * color[i][0].first;

        for (int j = 1; j < (int) color[i].size(); j++)
        {
            dist += 1LL * j * color[i][j].first - sum;
            sum += color[i][j].first;
        }

        sort(color[i].begin(), color[i].end(), sort2nd); // sort by col index
        sum = 1LL * color[i][0].second;

        for (int j = 1; j < (int) color[i].size(); j++)
        {
            dist += 1LL * j * color[i][j].second - sum;
            sum += color[i][j].second;
        }
    }
