...
void solve() {
    ...
    for (ll i = 1LL; i <= 10*n; i *= 10) // for each digit d in n
    {
        for (ll j = 1LL; j <= i; j *= 10) // for each digit up to d
        {
            for (int k = 0; k <= 9; k++) // try up to 9 times
            {
                if (ncpy % (i * 10) / i == 7) {ans = min(ans, k); break;}
                ncpy += 10*j - 1;
            }
