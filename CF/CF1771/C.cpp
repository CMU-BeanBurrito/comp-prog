...
void solve() {
    ...
    // any number greater than 1 occurs multiple times, clearly YES
    for (auto p : mp)
    {
    ...
    // all prime factors seen.
    // for each element in a, only add each of a[i]'s prime factors at most once each.
    // then, if for a prime p, pf[p] > 1, we know p was a factor of multiple elements, and the answer is YES
    map<int, int> pf;
