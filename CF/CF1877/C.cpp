...
void solve() {
    ...
    if (k > 3) // no options exist for this
    {
        printf("0\n");
    }
    else if (k == 1) // only 0 works 
    {
        printf("1\n");
    }
    else if (k == 2) // number divisible n, and numbers smaller than n (except 0)
    {
        if (m <= n)
        {
            printf("%d\n", m);
    ...
    else if (k == 3) // all options (m+1), except options from k==1 and k==2
    {
        if (m <= n)
        {
            printf("0\n");
