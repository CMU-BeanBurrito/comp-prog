 ...
 * Strategy: 
 * 
 * If the river is length 1, the answer is 1.
 * 
 * When can we NOT sail infinitely?
 * 
 * We will have some number of <, followed by at most one *, followed by some number of >.
 * If there is a *, we start there and go towards the more distant shore.
 * Otherwise, we start at the beginning of the current going towards the more distant shore.
 * - e.g.: <<<<<>>, start at the rightmost left current.
 * 
 * Any other case means we can infinitely sail, as one of the following will exist, any of which we can stay in:
 *                  >_____<
 *                  >*
 *                  *<
 *                  *_____*
