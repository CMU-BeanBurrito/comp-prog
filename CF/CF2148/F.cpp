...
/**
 * Strategy: Construct answer one "chunk" at a time.
 * 
 * How to acquire the next chunk?
 * 
 * Sort vectors amongst each other (these vectors are the elements of a).
 * Append the minimum vector c to answer. This is the chunk.
 * Discard all vectors with length less than or equal to len(c) (including c).
 * Delete prefix of len(c) elements from all other vectors.
 * 
 * Once a is empty we are done.
 */
...
void solve() 
{
    ...
    {
        sort(a.begin(), a.end());
        for (int i = 0; i < (int)a[0].size(); i++) ans.push_back(a[0][i]);
        ...
        for (auto v : a)
        {
            ...
            {
                vector<int> vtmp;
                vtmp.assign(v.begin()+a[0].size(), v.end());
                tmp.push_back(vtmp);
            }
        }
