...
void solve() {
    ...
    int ans = n+m; // we can just concatenate a+b at worst

    for (int i = 0; i < m; i++)
    {
        int bptr = i; // start matching a at this point in b
        ...
        // we found bptr-i of a's characters in s[i:m), so our answer is n+m-(bptr-i).
        ans = min(ans, n+m-(bptr-i));
    }
