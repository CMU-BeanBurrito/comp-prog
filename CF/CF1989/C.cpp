...
void solve() {
    ...
    int ar = 0; // a rating
    int br = 0; // b rating
    int bad = 0; // viewers who disliked both movies
    int good = 0; // viewers who liked both movies
    ...
    // use good/bad to balance out ar and br at this point
    int ans = INT_MIN;

    for (int i = -bad; i <= good; i++)
    {
        ans = max(ans, min(ar+i, br+good-bad-i));
    }
