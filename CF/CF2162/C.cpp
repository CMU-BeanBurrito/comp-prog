 ...
 * Strategy:
 * 
 * Note: "MSB" refers to the most significant bit in a number that has value of 1.
 * 
 * If MSB of b is highest than MSB of a, there is no solution as we can never toggle that higher bit in a.
 * 
 * Otherwise, simply go bit by bit and XOR if necessary. With this strategy, we will just XOR a with various powers of 2.
 * 
 * Lastly, sort the list of operations so it is in ascending order. This way, each operation is <= a.
 */
...
void solve() {
    ...
    // if the MSB of b is higher than the MSB of a, there is no solution.
    while(bit >= 0)
    {
        int mask = 1 << bit;

        // MSB of a is greater than or equal to MSB of b.
        if ((a & mask) > 0) break;
        
        // MSB of b is greater than MSB of a, there is no solution.
        if ((b & mask) > 0)
        {
            printf("-1\n");
            return;
    ...
    // collect bits where a and b differ. 
    while(bit >= 0)
    {
        int mask = 1 << bit;

        if ((a & mask) != (b & mask)) ans.push_back(mask);
        bit--;
    }
