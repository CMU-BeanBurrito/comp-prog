...
// PUT GLOBALS HERE
ll isqrt(ll x) // x must be positive
{
    ll left = 0LL;
    ll right = 3'037'000'499LL;
...
void solve() {
    ...
    /**
     * Discriminant for a parabola/line system is (b-k)^2 - 4ac
     * 
     * If 4ac is non-positive, parabola and line touch or intersect.
     * 
     * So, if 4ac is non-positive, no line will work.
     * 
     * Otherwise, (b-k)^2 must be strictly less than 4ac.
     */
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        auto left = lower_bound(k.begin(), k.end(), b[i]-rt);
        auto right = upper_bound(k.begin(), k.end(), b[i]+rt);

        // any lines in [left, right) will work
