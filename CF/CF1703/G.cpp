...
void solve() {
    ...
    ll ans = max(0LL, pref[n-1] - 1LL*n*k); // no bad keys purchased

    for (int i = 0; i < n; i++) // start buying bad keys at i
    {
        ll coins = pref[i] - a[i] - 1LL*i*k; // coins after opening all chests before i with good keys

        for (int j = i; j < min(n, i + 30); j++) // after using 30 bad keys, all remaining chests will have 0 coins left
        {
            coins += (a[j] >> (j-i+1));
        }
