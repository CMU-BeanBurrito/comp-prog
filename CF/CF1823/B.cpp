...
void solve() {
    ...
    int wrong = 0; // number of elements that need to be moved

    for (int i = 1; i <= n; i++)
    {
        // this if statement checks if this element is in a valid spot. for example, if k = 3, element 1 should be at index 1, 4, 7, 10.... (using 1-indexing)
        if ((a[i] % k) != (i % k)) wrong++;
    }

    if (wrong == 0) // no elements need to be moved, so 0 swaps necessary.
    {
        printf("0\n");
    }
    else if (wrong <= 2) // we can correct up to 2 elements with one swap. 
    {
        printf("1\n");
    }
    else // there are more than 2 elements that need moved, so we need more than 1 swap.
    {
        printf("-1\n");
    }
}
