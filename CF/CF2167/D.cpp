...
void solve() {
    ...
    if (g == 1) // there was an odd number. gcd(odd, 2) == 1, so x = 2 works
    {
        printf("2\n");
    }
    else if (g == 2) // can't use 2, as all numbers had 2 as a factor. but at least one did not have 3. so use x = 3.
    {
        printf("3\n");
    }
    else // "brute force". we know this while loop will terminate quickly as the product of the first few dozen primes will exceed 1E18
    {
        ll x = 2;
        while(gcd(g, x) != 1) x++;
