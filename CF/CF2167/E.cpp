...
void solve() {
    ...
    vector<pair<int, int>> g; // best spot on the left
    if (a[0] != 0) g.push_back(make_pair(0, a[0]-1));
    if (a[n-1] != x) g.push_back(make_pair(x, a[n-1]+1));
    ...
    for (int i = 1; i < n; i++)
    {
        ...
        if (left < mid) g.push_back(make_pair(mid, left+1));
        if (mid+1 < right) g.push_back(make_pair(mid+1, right-1));
    }
    ...
    if (g.size() > 0)
    {
        sort(g.begin(), g.end(), sortpairdiff);

        while((int)ans.size() < k)
        {
            int h = abs(g[0].first-g[0].second)+1;

            if (g[0].first == INT_MAX) break;

            for (int i = 0; i < (int)g.size() && (int)ans.size() < k; i++)
            {
                if (g[i].first == INT_MAX) break;

                if (abs(g[i].first-g[i].second)+1 == h)
                {
