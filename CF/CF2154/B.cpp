 ...
 * Strategy:
 * 
 * Perform operation 1 on all even indices (odd indices if using 0-indexing, which this implementaion does).
 * 
 * From there, traverse odd indices (or even if 0-indexing), compare to neighboring indices to see how much to subtract.
 * - for indices to the right, this will be at most 1 (since we can use operation 1 to at least become equal to the previous element)
 */
...
void solve() {
    ...
    for (int i = 0; i < n; i += 2)
    {
        ...
        if (i-1 > 0)
        {
            if (a[i] >= a[i-1]) sub = max(sub, a[i]-a[i-1]+1);
        }
