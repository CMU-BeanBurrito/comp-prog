...
void solve() {
    ...
    ll ans = 0LL;
    bool copy = false; // can we get b[n] in 1 operation?
    int closest = INT_MAX;

    for (int i = 0; i < n; i++)
    {
        ans += abs(a[i]-b[i]);
        if (a[i] <= b[n] && b[n] <= b[i]) copy = true;
        if (b[i] <= b[n] && b[n] <= a[i]) copy = true;
        closest = min(closest, abs(b[n]-b[i]));
        closest = min(closest, abs(b[n]-a[i]));
    }
