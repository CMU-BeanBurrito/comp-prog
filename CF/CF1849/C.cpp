...
void solve() {
    ...
    last0[1] = (s[0] == '0' ? 1 : 0); // last 0 in prefix ending at index
    vector<int> first1 (n+1);
    first1[n] = (s[n-1] == '1' ? n : n+1); // first 1 in suffix starting at index

    for (int i = 2; i <= n; i++)
    {
        last0[i] = (s[i-1] == '0' ? i : last0[i-1]);
    }

    for (int i = n-1; i >= 1; i--)
    {
        first1[i] = (s[i-1] == '1' ? i : first1[i+1]);
    }
    ...
    while(m--)
    {
        ...
        if (first1[left] < last0[right]) // segment not sorted, these two points contains all indices that will change
        {
            mp[make_pair(first1[left], last0[right])]++;
        }
        else // segment already sorted
        {
            mp[make_pair(-1, -1)]++;
        }
    }
    ...
    int ans = 0;
    for (auto p : mp) ans++; // count unique pairs
