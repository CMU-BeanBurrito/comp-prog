...
void solve() {
    ...
    // laser covering entire row
    if (abs(1-sy) <= d && abs(m-sy) <= d)
    {
        printf("-1\n");
        return;
    }

    // laser covering entire column
    if (abs(1-sx) <= d && abs(n-sx) <= d)
    {
        printf("-1\n");
        return;
    }

    // laser trapping robot in top left
    if (abs(1-sy) <= d && abs(1-sx) <= d)
    {
        printf("-1\n");
        return;
    }
    ...
    // laser encloses dest
    if (abs(n-sx) <= d && abs(m-sy) <= d)
    {
        printf("-1\n");
        return;
    }

    // otherwise there is a shortest path that is unblocked
    printf("%d\n", n-1 + m-1);
}
