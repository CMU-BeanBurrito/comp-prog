...
void solve() {
    ...
    // minimum balance value we can get is abs(n-m)
    // maximum we can get is max(n, m)
    ...
    if (k != mn)
    {
        if (n >= m)
        {
            while(n > 0 || m > 0)
            {
                for (int i = 0; i < min(n, k); i++) printf("0");
                for (int i = 0; i < min(m, k); i++) printf("1");
                n -= min(n, k); m -= min(m, k);
            }
        }
        else
    ...
    if (k == mn)
    {
        ...
        {
            int extra = n-m-1;
            for (int i = 0; i < extra/2; i++) printf("0");
            for (int i = 0; i < m; i++) printf("01");
            for (int i = 0; i < extra-(extra/2)+1; i++) printf("0");
        }
        else
        {
            int extra = m-n-1;
            for (int i = 0; i < extra/2; i++) printf("1");
            for (int i = 0; i < n; i++) printf("10");
            for (int i = 0; i < extra-(extra/2)+1; i++) printf("1");
        }
        printf("\n");
    }
