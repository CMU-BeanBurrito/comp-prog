...
// return least element that is greater than or equal to key
int binsearch(vector<int>& a, int n, int l, int r, int key)
{
    while(l < r)
    {
        int mid = (l+r)/2;
...
void solve() {
    ...
    for (auto p : bmp) b.push_back(p.first);
    ...
    {
        int key = x + a[i];
        int res = binsearch(b, b.size(), 0, b.size()-1, key);
