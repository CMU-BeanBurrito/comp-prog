...
// PUT GLOBALS HERE
bool xsort(const pair<int, int> &p1, const pair<int, int> &p2)
{
...
void solve() 
{
    ...
    vector<pair<int, int>> a (n); // balance, position
    ...
    {
        int d = s[i-1]  == '(' ? 1 : -1;
        a[i] = make_pair(a[i-1].first + d, i);
    }

    sort(a.begin(), a.end(), xsort);

    for (int i = 0; i < n; i++) printf("%c", s[a[i].second]);
    printf("\n");
}
