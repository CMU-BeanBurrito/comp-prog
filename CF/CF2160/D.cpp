...
void solve() {
    ...
    for (int i = 1; i <= 2*n; i++)
    {
        ...
        // query our working set of indices.
        printf("? %d ", (int)s.size());
        ...
        // found a duplicate. record the element we found, but do not include the index in the working set.
        if (resp != 0)
        {
            a[i] = resp;
            s.erase(s.find(i));
    ...
    // construct t, which holds all indices not in s.
    set<int> t;
    for (int i = 1; i <= 2*n; i++)
    {
    ...
    // we have found the second occurrence of each element and recorded it in a.
    // s also contains all indices whose elements are unknown.
    // each query in this phase will be one index from s, plus all indices in t.
    for (int x : s)
    {
        printf("? %d ", n+1);
        printf("%d ", x);
        for (int y : t) printf("%d ", y);
        printf("\n");
