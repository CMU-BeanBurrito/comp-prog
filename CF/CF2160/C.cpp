 ...
 * 
 * For some bit in n:
 * - if bit is 0, the corresponding bit in x must equal the corresponding bit in f(x)
 * - if bit is 1, the corresponding bit in x must not equal the corresopnding bit in f(x)
 * - therefore, the bits of n must form a palindrome
 *   - leading 0s can be used to create this palindrome. if x and f(x) have leading 1s, it will create a leading 0 in n.
 *   - an equivalent way of handling this is to ignore trailing 0s.
 * 
 * - we cannot have a single '1' as the pivot in the palindrome, as a bit cannot differ from itself.
 * 
 * So, we trim leading and trailing 0s from the bit level representation of n.
 * If the remaining bit string is a palindrome, the answer is YES.
 * - exception: the remaining bit string is odd length and the middle is a '1'. in this case, the answer is NO.
 * 
...
void solve() {
    ...
    // edge case. first, last won't be init correctly
    if (n == 0)
    {
        printf("YES\n");
        return;
    ...
    // iterate from first, but stop before the midpoint (if odd length)
    for (int i = first; i < (first+last+1)/2; i++)
    {
        if (bits[i] != bits[last-(i-first)])
        {
            printf("NO\n");
            return;
        }
