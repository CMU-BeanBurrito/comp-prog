...
void solve() {
    ...
    r = a;
    c = m;
    while(r > 1) {r = divceil(r, 2); below++;}
    while(c > 1) {c = divceil(c, 2); below++;}

    int right = 1;
    r = n;
    c = b;
    while(r > 1) {r = divceil(r, 2); right++;}
    while(c > 1) {c = divceil(c, 2); right++;}

    int above = 1;
    r = n-a+1;
    c = m;
    while(r > 1) {r = divceil(r, 2); above++;}
    while(c > 1) {c = divceil(c, 2); above++;}

    int left = 1;
    r = n;
    c = m-b+1;
    while(r > 1) {r = divceil(r, 2); left++;}
    while(c > 1) {c = divceil(c, 2); left++;}
   
    printf("%d\n", min(min(above, below), min(left, right)));
}
