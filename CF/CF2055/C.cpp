...
void solve() 
{
    ...
    vector<pair<int, int>> path (n+m-1);
    path[0] = make_pair(0, 0);
    path[n+m-2] = make_pair(n-1, m-1);
    ...
    {
        if (s[i-1] == 'R') path[i] = make_pair(path[i-1].first, path[i-1].second+1);
        if (s[i-1] == 'D') path[i] = make_pair(path[i-1].first+1, path[i-1].second);
        rmiss[path[i].first]++;
        cmiss[path[i].second]++;
    }

    vector<ll> rsum (n, 0LL);
    vector<ll> csum (m, 0LL);
    ...
    for (auto p : path)
    {
        ...
        if (rmiss[p.first] == 1)
        {
            add = 0LL - rsum[p.first];
        }
        else
        {
            add = 0LL - csum[p.second];
        }

        a[p.first][p.second] = add;
        rsum[p.first] += add;
        csum[p.second] += add;
        rmiss[p.first]--;
        cmiss[p.second]--;
