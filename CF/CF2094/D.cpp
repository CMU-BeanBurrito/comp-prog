...
void solve() {
    ...
    while(pptr < p.length() && sptr < s.length())
    {
        if (p[pptr] != s[sptr])
        {
            printf("NO\n");
        ...
        while(pptr < p.length() && p[pptr] == c) { pptr++; pcnt++; }
        while(sptr < s.length() && s[sptr] == c) { sptr++; scnt++; }

        if (scnt < pcnt || 2*pcnt < scnt)
        {
            printf("NO\n");
            return;
        }
    ...
    if (pptr != p.length() || sptr != s.length())
    {
        printf("NO\n");
    }
    else
