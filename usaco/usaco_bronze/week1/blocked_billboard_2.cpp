...
int x_intersect(vector<int>& a, vector<int>& b)
{
    if (a[2] <= b[0]) return 0;
    if (b[2] <= a[0]) return 0;
    int result = min(abs(b[2]-a[0]), abs(a[2]-b[0]));
    return min(result, min(b[2]-b[0], a[2]-a[0]));
}

int y_intersect(vector<int>& a, vector<int>& b)
{
    if (a[3] <= b[1]) return 0;
    if (b[3] <= a[1]) return 0;
    int result = min(abs(b[3]-a[1]), abs(a[3]-b[1]));
    return min(result, min(b[3]-b[1], a[3]-a[1]));
}
...
void solve() {
    ...
    else if ((tr && tl) || (br && bl)) // some top part is covered or some bottom part is covered
    {
        printf("%d\n", (back[2]-back[0])*(back[3]-back[1]-y_intersect(front, back)));
    }
    else if ((tl && bl) || (tr && br)) // some left part or some right part is covered
    {
        printf("%d\n", (back[2]-back[0]-x_intersect(front, back))*(back[3]-back[1]));
    }
    else // at most one corner is covered, so we need enough tarp to cover the entire lawnmower billboard
    {
