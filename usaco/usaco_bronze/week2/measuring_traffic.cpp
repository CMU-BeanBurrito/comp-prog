...
void solve() {
    ...
    for (int i = 0; i < n; i++) cin >> sensors[i].first >> sensors[i].second.first >> sensors[i].second.second;
    ...
    for (int i = 0; i < n; i++)
    {
        if (sensors[i].first == "none")
        {
            fmn = max(fmn, sensors[i].second.first);
            fmx = min(fmx, sensors[i].second.second);
        }
        else if (sensors[i].first == "on")
        {
            fmn += sensors[i].second.first;
            fmx += sensors[i].second.second;
        }
        else
        {
            fmn -= sensors[i].second.second;
            fmn = max(0, fmn);
            fmx -= sensors[i].second.first;
        }
    }
    ...
    for (int i = n-1; i >= 0; i--)
    {
        if (sensors[i].first == "none")
        {
            imn = max(imn, sensors[i].second.first);
            imx = min(imx, sensors[i].second.second);
        }
        else if (sensors[i].first == "on")
        {
            imn -= sensors[i].second.second;
            imn = max(0, imn);
            imx -= sensors[i].second.first;
        }
        else
        {
            imn += sensors[i].second.first;
            imx += sensors[i].second.second;
        }
    }
