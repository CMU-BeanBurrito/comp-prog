...
void solve() {
    ...
    int n; cin >> n;
    vector<vector<int>> boards (n+1, vector<int> (3));
    ...
    for (int i = 1; i <= n; i++)
    {
        ...
        boards[i][1] = boards[i-1][1];
        boards[i][2] = boards[i-1][2];

        int a, b, g; cin >> a >> b >> g;
        boards[i][a-1] = boards[i-1][b-1];
        boards[i][b-1] = boards[i-1][a-1];

        if (boards[i][g-1] == 1) correct1++;
        if (boards[i][g-1] == 2) correct2++;
        if (boards[i][g-1] == 3) correct3++;
    }
    
    printf("%d\n", max(correct1, max(correct2, correct3)));
}
